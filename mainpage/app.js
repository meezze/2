  ...
  methods: {
    ...
    addToCart(item) {
      if (!item.inCart) {
        ...
        // this.$nextTick(() => {
        //     this.$refs[`cartImageWrapper${cartItem.id}`].classList.remove("hidden");
        //     this.$refs[`cartText${cartItem.id}`].classList.remove("hidden");
        //     this.$refs[`cartPrice${cartItem.id}`].classList.remove("hidden");
        //   });
        ...
        tl.add(
          gsap.from(
            [
              ...
              this.$refs[`sizeLabel${itemId}`],
              this.$refs[`selectSize${itemId}`],
            ],
            {
              duration: 0.5,
              x: 30,
              ...
              stagger: 0.15,
              ease: "power4",
              clearProps: 'opacity', // Удалите это, если оно больше не нужно для label и select
            }
          ),
          "-=0.25"
        );
